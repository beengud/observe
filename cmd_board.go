...
func readBoardInput(fa FuncArgs, filePath string) (map[string]any, error) {
	...
	if stages, ok := input["stages"].([]any); ok {
		for _, s := range stages {
			...
			// InputDefinitionInput fields that are nullable on write but non-null
			// on read (InputDefinition return type). Omitting them causes the
			// API to store null; any query requesting those fields as String!/
			// InputRole! then fails with "requested element is null which
			// schema does not allow."
			//
			//   stageId:   String!  — use "" for dataset inputs
			//   inputRole: InputRole! — use "Data" for dataset inputs
			if inputs, ok := stage["input"].([]any); ok {
				for _, inp := range inputs {
					...
					if _, has := entry["stageId"]; !has {
						entry["stageId"] = ""
					}
					if _, has := entry["inputRole"]; !has {
